...
from common import RANDOM_VIDEO_TIME, SEARCH_LIMIT, YOUTUBE_VIDEO_SHAREABLE_LINKS
...
class Browser(metaclass=abc.ABCMeta):
    ...
    def setup_options(self):
        """
        This function returns the options for the browser driver.
        """
        options = None
    ...
    def treat_video(self, inner_elements, video_title: str, video_time: str, channel: str, search_type: SearchEnum = SearchEnum.SEARCH):
        """
        ...
        for inner_element in inner_elements:
            ...
            if video_title in title and channel in video_info["channel"].lower().strip():
                ...
                sleep(5)
                video_time = youtube_time_to_seconds(video_time)
                self.playback_speed(15)
                sleep(video_time / 15)
                break

    @SE.handle_exceptions
    def play_searched_video(self, video_time: str, channel: str, search_type: SearchEnum = SearchEnum.SEARCH, video_title: str = " ",):
        """
        ...
        while not self.video_found and self.limit < SEARCH_LIMIT:
            ...
            if search_type in (SearchEnum.SEARCH, SearchEnum.HOME):
                ...
                for element in merged_list:
                    ...
                    self.treat_video(inner_elements, video_title,
                                     video_time, channel, search_type)
        ...
        if not self.video_found and self.limit >= SEARCH_LIMIT:
            if search_type == SearchEnum.SEARCH:
                self.searched_channel_video(
                    video_title, video_time, '@NatureHealingHaven')
    ...
    def treat_channel_video(self, channel_videos, video_title: str, video_time: str, channel: str):
        """
    ...
    def searched_channel_video(self, video_title: str, video_time: str, channel: str):
        """
        ...
        self.treat_channel_video(
            channel_videos, video_title, video_time, channel)
        # self.not_found_reset()
        # self.after_found_reset()
    ...
    def get_random_channel_video(self, channel: str):
        ...
        """
        channel_videos = self.get_channel_videos(channel)
        self.treat_channel_video(channel_videos, '', RANDOM_VIDEO_TIME, '')
        self.not_found_reset()
        self.after_found_reset()
    ...
    def search_video_info(self, element, search_type: SearchEnum = SearchEnum.SEARCH) -> VideoInfo:
        ...
        try:
            thumbnail = element.find_element(By.CSS_SELECTOR if search_type in (SearchEnum.SEARCH, SearchEnum.RELATED) else By.XPATH,
                                             "#dismissible > ytd-thumbnail > a#thumbnail" if search_type in (SearchEnum.SEARCH, SearchEnum.RELATED) else '//a[@id="thumbnail"]')

            text_wrapper = element.find_element(By.CSS_SELECTOR if search_type in (
                SearchEnum.SEARCH, SearchEnum.RELATED) else By.XPATH, "#dismissible > div" if search_type in (SearchEnum.SEARCH, SearchEnum.RELATED) else '//div[@id="details"]')
        except TimeoutException:
    ...
    def wait(self, condition: EC, time=20):
        """
